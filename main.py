...
parser = argparse.ArgumentParser(
  description='Download videos off the WWE Network.')
parser.add_argument('-t', '--title',
                    help='Link of the video you want to download. Example: https://network.wwe.com/video/178225',
                    required=True)
parser.add_argument('-q', '--quality',
                    help='Quality of the video you wish to download. Value between 1 (highest) and 6 (lowest). Defaults to 1080p.',
                    required=False)
parser.add_argument('-c', '--chapter',
                    help='Add chapter "milestones" to the video.',
                    required=False, action='store_true')
parser.add_argument('-s', '--subtitles', help='Add subtitles to the video.',
                    required=False, action='store_true')
parser.add_argument('-k', '--keep_files',
                    help='Keep the temporary download files.', required=False,
                    action='store_true')
parser.add_argument('-st', '--start_time',
                    help='How far into the video you want to start, in seconds. Note: Will overide other start points.',
                    required=False)
parser.add_argument('-et', '--end_time',
                    help='How far into the video you want to stop, in seconds.',
                    required=False)
parser.add_argument('-of', '--output_filename', help='Custom output file name.',
                    required=False)
...
if args['title']:
  ...
  if "https://network.wwe.com/video/" in EPISODE:
    ...
    print(
      "Invalid link format. It should contain \"https://network.wwe.com/video/\"")
    exit()
...
if args['quality']:
  if int(args['quality']) < 0 or int(args['quality']) >= len(
      CONSTANTS.VIDEO_QUALITY):
    print(
      f"Invalid quality choice. It must be between 0 (1080p) and {len(CONSTANTS.VIDEO_QUALITY)} (288p)")
    exit()
...
for i in index_m3u8_obj.media:
  ...
  if "eng" in i.language:
    audio_qualities.append(
        (int(i.group_id.split('audio-')[1]), base_url[0] + "/" + i.uri))
...
for i in index_m3u8_obj.playlists:
  ...
  if (i.stream_info.average_bandwidth <= QUALITY * 1000):
    ...
    video_selections.append(
        (i.stream_info.bandwidth, base_url[0] + "/" + i.uri))
...
if db_q:
  database.db_upd(EPISODE, video_link[0], str(video_selections[0][0]),
                  partial_download, int(time.time()))
  print("Updated database with the new video information")
else:
  database.db_ins(EPISODE, video_link[0], str(video_selections[0][0]),
                  partial_download, int(time.time()))
