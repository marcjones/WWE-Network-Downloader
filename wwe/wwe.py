...
class wwe_network:
  ...
  def refresh_token(self):
    if not self.refreshToken:
      ...
      return
    xx = self._session.post(
      'https://dce-frontoffice.imggaming.com/api/v2/token/refresh',
  ...
  def login(self):
    ...
    token_data = self._session.post(
      'https://dce-frontoffice.imggaming.com/api/v2/login', json=payload,
  ...
  def write_metadata(self, link, episode_title, chapterize=False, start=None,
      end=None):
...
title={episode_title}\n")
    ...
    if chapterize:
      ...
      for i in chapters:
        ...
        if (time_to_seconds(timestamp_end) >= start and (
            end == 0 or time_to_seconds(timestamp_end) <= end)):
          print(
            f"{html.unescape(i[1])}: {time_to_seconds(timestamp_start)} - {time_to_seconds(timestamp_end)}")
          meta_file.write(
  ...
  def get_video_info(self, link):
    ...
    api_link = self._session.get(
      f'https://dce-frontoffice.imggaming.com/api/v4/vod/{link}?includePlaybackDetails=URL').json()
